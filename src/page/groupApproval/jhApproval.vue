      ...
      <mt-loadmore v-else class="list-selector" :top-method="loadTop" :bottom-method="loadBottom"
        ...
        <mt-tab-container v-model="selected">
          <mt-tab-container-item id="1">
            <div id="todo">
              <jhApproval-list v-for='(item,index) in dealtInfo'
                                   :key="index"
...
<script>
  ...
  import JhApprovalList from '@/components/workFlow/jhApproval-list.vue'
  ...
  export default {
    ...
    components: {HeaderSimple, BodyContent, JhApprovalList},
    methods: {
      goBack() {
        this.$router.push({path: '/group'})
      },
      //获取行政审批列表
      getJhApprovalList(type, params) {
        let that = this;
        that.dealtInfo = [];
//        that.showIndicator('加载中...');
        interfaceService.queryJhApprovalList(type, params).then(function (response) {
          that.hideIndicator();
...
//        that.showIndicator('加载中...');
      ...
      loadTop() {
        ...
        setTimeout(() => {
          if (self.selected === '1') {
            self.getJhApprovalList('待办', self.params);
          } else if (self.selected === '2') {
            self.getJhApprovalList('已办', self.params);
          } else if (self.selected === '3') {
            self.getJhApprovalList('办结', self.params);
          }
      ...
      selected(newval, oldval) {
        ...
        switch (newval) {
          case '1':
            ...
            this.getJhApprovalList('待办', this.params);
            break;
          case '2':
            this.showIndicator('加载中...');
            this.getJhApprovalList('已办', this.params);
            break;
          case '3':
            this.showIndicator('加载中...');
            this.getJhApprovalList('办结', this.params);
            break;
          default:
            break;
        }
    ...
    activated() {
      if (this.$route.query.page !== 'jhApprovalDetail' || this.is_weixin()) {
        this.selected = '1';
        this.params = {
          "from": "0",
          "limit": "10"
