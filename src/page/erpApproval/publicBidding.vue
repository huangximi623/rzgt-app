      ...
      <mt-loadmore v-else class="list-selector" :top-method="loadTop" :bottom-method="loadBottom"
        ...
        <mt-tab-container v-model="selected">
          <mt-tab-container-item id="1">
            <div id="todo">
              <publicBidding-list v-for='(item,index) in dealtInfo'
                                   :key="index"
...
<script>
  ...
  import PublicBiddingList from '@/components/workFlow/publicBidding-list.vue'
  ...
  export default {
    ...
    components: {HeaderSimple, BodyContent, PublicBiddingList},
    methods: {
      goBack() {
        this.$router.push({path: '/erp'})
      },
      //获取行政审批列表
      getPublicBiddingList(type, dataValue, params) {
        let that = this;
        that.dealtInfo = [];
//        that.showIndicator('加载中...');
        interfaceService.queryERPList(type, dataValue, params).then(function (response) {
          that.hideIndicator();
      ...
      //加载更多
      loadmore(type, dataValue, params) {
        let that = this;
//        that.showIndicator('加载中...');
        interfaceService.queryERPList(type, dataValue, params).then(function (response) {
          if (response) {
      ...
      loadTop() {
        ...
        setTimeout(() => {
          if (self.selected === '1') {
            self.getPublicBiddingList('待办', 'MQE', self.params);
          } else if (self.selected === '2') {
            self.getPublicBiddingList('已办', 'MQE', self.params);
          } else if (self.selected === '3') {
            self.getPublicBiddingList('办结', 'MQE', self.params);
          }
      ...
      loadBottom() {
        ...
        setTimeout(() => {
          if (self.selected === '1') {
            self.loadmore('待办', 'MQE', self.params);
          } else if (self.selected === '2') {
            self.loadmore('已办', 'MQE', self.params);
          } else if (self.selected === '3') {
            self.loadmore('办结', 'MQE', self.params);
          }
      ...
      selected(newval, oldval) {
        ...
        switch (newval) {
          case '1':
            ...
            this.getPublicBiddingList('待办', 'MQE', this.params);
            break;
          case '2':
            this.showIndicator('加载中...');
            this.getPublicBiddingList('已办', 'MQE', this.params);
            break;
          case '3':
            this.showIndicator('加载中...');
            this.getPublicBiddingList('办结', 'MQE', this.params);
            break;
          default:
            break;
        }
    ...
    activated() {
      if (this.$route.query.page !== 'publicBiddingDetail' || this.is_weixin()) {
        this.selected = '1';
        this.params = {
          "from": "0",
          "limit": "10"
